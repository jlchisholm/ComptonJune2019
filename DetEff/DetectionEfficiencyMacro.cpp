...
void GetDetEff(int ke)
{
	...
	// Input file (made by EvGen)
        TFile *evgenFile = TFile::Open(Form("~/opt/EvGen/out/5cm/compton_he4_%d_in.root",ke));
	if(!evgenFile->IsOpen())
	{
	...
	// Output file (from A2Geant4 and Ant)
        TFile *antFile = TFile::Open(Form("MC_for_Edet/he4compton_%d.root",ke));
	if(!antFile->IsOpen())
	{
	...
	TH1D *hThetaIn = (TH1D*)evgenFile->Get("h4");
        TH3D *h3DOut = (TH3D*)antFile->Get("He4Compton/h3D_ME1");
